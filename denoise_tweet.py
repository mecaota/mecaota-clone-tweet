...
# in_reply_to_status_idとin_reply_to_user_idの値が入っているツイートを選別。０ならリプライ除外、それ以外はリプライのみ
...
if __name__ == "__main__":
    ...
    tweets = tweet_filter(tweets, 0)
    replies = tweet_filter(tweets, 0)

    print(tweets)

    # csv export
    tweets.to_csv("tweets_shaped.csv")
    repliess.to_csv("replies_shaped.csv")